...
export class ArM5eItem extends Item {
  ...
  prepareData() {
    ...
    if (this.type == "magicalEffect") {
      ...
      if (data.range.value) {
        effectLevel = this._addSpellMagnitude(effectLevel, CONFIG.ARM5E.ranges[data.range.value].impact);
      }
      if (data.duration.value) {
        effectLevel = this._addSpellMagnitude(effectLevel, CONFIG.ARM5E.durations[data.duration.value].impact);
      }
      if (data.target.value) {
        effectLevel = this._addSpellMagnitude(effectLevel, CONFIG.ARM5E.targets[data.target.value].impact);
      }
      if (data.complexity) {
